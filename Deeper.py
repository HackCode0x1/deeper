...
def Clean_Gen_Files():
	temp_file = 'temp.txt'
	temp_file_1 = 'temp1.txt'
	temp_F = 'temp2.txt'
	Gen_Files = [temp_file,temp_file_1,temp_F]
	for f in Gen_Files:
		if os.path.exists(f):
			try:
				os.remove(f)
...
def SPAN_EQ(stop):
	...
	direction = 1
	while True:
	    print("\r[{}={}]".format(" " * position, " " * (size - position), position), end='')
	    sys.stdout.flush()
...
def helpmenu():
	...
	print("{:<25}{}".format('list',"To List options"))
	print()
def options():
	...
	print("crackmethod : word             :  {}    choice Crack Hash Method [crackhash] Mode ".format(t))
	...
	print("verbose     : bool             :  {}   verbose Cracking Passwords Default False set in [crackhash] mode slows down cracking time ".format(f))
	print()
...
def list(option):
	...
	Modes = ['enc: encryption Password','dec: decryption Password','hash: Hashing Password','hashhmac: Hashing hmac Password','crackhash: Crack Hash']
...
def run(algorithm,PASSWORD,MODE,saveplace,publickey,privatekey,keylength,HashSalt,wordlist,Crackmethod,Min,Max,Chars,Verbose):
	...
	if MODE=='hash':
		...
		"""	
		private_key = RSA.generate(int(keylength))
		...
		print();print(public_pem);print()
		privatepath = saveplace+'private_pem.pem'
		publickeypath = saveplace+'public_pem.pem'
		with open(privatepath, 'w') as pr:
		    pr.write(private_pem)
		with open(publickeypath, 'w') as pu:
		    pu.write(public_pem)
		"""
			
		    
		#Importing keys from files, converting it into the RsaKey object
		#Priv = saveplace+'\\private.pem'
		#Pup = saveplace+'\\public.pem'
		Priv  = privatekey
		Pup = publickey
		try:
		...
		if Crackmethod=='dictionary':
			...
			for i in readwordlist:
			    ...
			    if HASH in Hashing:
			      Cracked_Hash = i
			      solved = True
			      print('[+] Hash Cracked!: {}:{} '.format(HASH,Cracked_Hash))
			      break
			    else:
			    	if verbose=='True':
			    		print('Try:'+HASH+':',i)
...
def Banner(SHELL=None):
	...
	VE = Fore.LIGHTCYAN_EX+'        Version'+Style.RESET_ALL+":"+Fore.RED+'  3.0.0'+Style.RESET_ALL
	...
	B1 = """
 _    ____ _  _ ____    _  _ ____    _  _ ____ ____ ____
...
""".format(T2)
	...
	if B4=="":
		Banners = [Fore.CYAN+B1+Style.RESET_ALL,Fore.CYAN+B2+Style.RESET_ALL,B3+Style.RESET_ALL,Fore.LIGHTRED_EX+B5+Style.RESET_ALL,BSY1,Down_Ban]
	
	if B5 =="":
		Banners = [Fore.CYAN+B1+Style.RESET_ALL,Fore.CYAN+B2+Style.RESET_ALL,B3+Style.RESET_ALL,Fore.LIGHTRED_EX+B4+Style.RESET_ALL,BSY1,Down_Ban]
...
|             {}	          |
 ------------------------------------------
	...
	if os.path.exists(PubKey) and os.path.exists(PrKey) :
		...
		print(ast,'Press [Enter] To Return To Menu...',end="");input()
  ...
  **********************************************************
	...
	if os.path.isdir(Data_Path):
		...
		if Data_Path.split('.')[-1]=='aes':
			print(Mrak,'Error already Encrypted File [aes]')
	...
	if os.path.exists('encrypted_data.bin'):
		print(I,'Warning Do You Want To Overwrite encrypted_data.bin This File Contains Encrypted Password[Y|N]',end="");x=input()
		if x=='y':
	...
	try:
		if Data=="Folder":
		    ...
		    'go', 'pyc', 'bf', 'coffee', 
			'jpg', 'jpeg', 'bmp', 'gif', 'png', 'svg', 'psd', 'raw','webp',
			'mp3','mp4', 'm4a', 'aac','ogg','flac', 'wav', 'wma', 'aiff', 'ape',
			'avi', 'flv', 'm4v', 'mkv', 'mov', 'mpg', 'mpeg', 'wmv', 'swf', '3gp','ts,', 
			...
			for path, folders, files in os.walk(parent):
				for f in files:
					...
					bad_chars = [r' ', r',', r'-', r'&', r'[', r']', r'(', r')', r'__',r'  ',r'   ',r'    ',r'     ',r';']
			...
			if verbose=='false':
				stop_threads = False 
				t1 = threading.Thread(target = PRGRES_BAR_3, args =(len(FINAL_FILES),'enc',lambda : stop_threads, ), daemon=True) 
				t1.start()
						...
						with open(i, 'rb') as infile:
							with open(outputfile, 'wb') as outfile:
								...
								if verbose=='false':
									...
									#sys.stdout.write("\rEncrypting  {} ".format(File_Name))
									#sys.stdout.write("\033[F")
									#CURSOR_UP_ONE = '\x1b[1A' 
									#ERASE_LINE = '\x1b[2K' 
									#sys.stdout.write(CURSOR_UP_ONE) 
			...
			if  ENC_CODE ==0:
				...
				print(ast+' Taken Time '+"{:0>2}:{:0>2}:{:05.2f}".format(int(hours),int(minutes),seconds))
				print()
				...
				print()
				print(ast,'PRESS [ENTER] TO CONTINUE ',end="") ;input()
				#clear()
				sys.exit()
			else:
				print(ast,'PRESS [ENTER] TO CONTINUE ',end="") ;input()
...
def Decryption():
	if os.path.exists('encrypted_data.bin') and os.path.exists('public.pem'):
		...
		print()
		print('[!] PRESS ENTER TO RETURN ',end="");input()
		main()
		sys.exit()
	...
	if os.path.isdir(Data_Path)==True:
		...
		Data = 'File'
			...
			#pass
		#else:
			#print(Mrak,'Error {} Not Encrypt File '.format(Data_Path))
			#print(I+' PRESS Enter To Return..',end='') ; input()
	...
	try:
		...
		if Data=='Folder':
			...
			if verbose=='false':
				stop_threads = False 
				t1 = threading.Thread(target = PRGRES_BAR_3, args =(len(files),'dec',lambda : stop_threads, ), daemon=True) 
				t1.start()
...
def Banners_Style():
	...
	VE = Fore.LIGHTCYAN_EX+'        Version'+Style.RESET_ALL+":"+Fore.LIGHTRED_EX+'  3.0.0'+Style.RESET_ALL


	b1t = Fore.LIGHTRED_EX+'DC:\> PyCrypto'+Style.RESET_ALL
	B1 = """
	   /+===============+\ 
	   || {}|| 
	   ||               || 
...
'''.format(D,CN,D,D,CB,D,D,VE,D) 
	...
	CODE_BANNERV = '['+Fore.GREEN+'--'+Style.RESET_ALL+"]  "+Fore.LIGHTCYAN_EX+'         Version'+Style.RESET_ALL+":"+Fore.LIGHTRED_EX+'  3.0.0'+Style.RESET_ALL+'              ['+Fore.GREEN+'--'+Style.RESET_ALL+"]"
 ...
 -@@,                <>                .@@=
  ...
  =@@,        :@@@@@@@@@@@@@@@@@@@@@= .@@:
   +@X        :@@@@@@@@@@@@@@@M@@@@@@:%@%
	...
	Black_mesa,chappie,SYSFIL,B11,Seclogin,prg2,PR1_w,Error_Bannr]
	print(random.choice(Banners))
		...
		try:
			...
			if ch==1:
				...
				while True:
					...
					if shell[0]=='run':
						if mode=='hash':
							if algorithm=='':
								...
								run(algorithm,password,'hash',False,False,False,False,False,False,False,False,False,False,False)
							...
							if password!='' and publickey!='' and privatekey!='':
								run(False,password,'enc',saveplace,publickey,privatekey,False,False,False,False,False,False,False,False)
							...
							if password!='' and publickey!='' and privatekey!='':
								run(False,password,'dec',saveplace,publickey,privatekey,False,False,False,False,False,False,False,False)
								...
								if algorithm=='':
									...
									run(algorithm,password,'hashhmac',False,False,False,False,False,False,False,False,False,False,False)
							...
							if Crackmethod =='':
								...
								if Crhash !='' and HashSalt!='' and wordlist!='':
									run(False,Crhash,'crackhash',False,False,False,False,HashSalt,wordlist,Crackmethod,False,False,False,verbose)
					...
					Commands = ['Set','run','list','help','options','exit']
